...
def plot_risk_return_scatter(annual_metrics_dict: Dict[str, Dict[str, float]], portfolio_metrics: Dict[str, float]):
    ...
    st.altair_chart(scatter, use_container_width=True)
def monte_carlo_simulation_portfolio(aligned_returns: pd.DataFrame, weights: list, horizon: int = 12, n_sims: int = 1000) -> pd.DataFrame:
    """
    ...
    aligned_returns : pd.DataFrame
        Historical return data for the assets, with each column representing an asset.
    weights : list of float
        Portfolio weights for each asset (should sum to 1).
    horizon : int, optional
        Forecast horizon in periods (e.g., months). Default is 12.
    n_sims : int, optional
    ...
    --------
    pd.DataFrame
        A DataFrame of simulated cumulative portfolio returns. Each row is a simulation, and each
        column corresponds to a forecast period.
    """
    # Input validations
    if aligned_returns.empty:
    ...
    # Compute historical mean returns and covariance matrix
    mean_vector = aligned_returns.mean().values
    cov_matrix = aligned_returns.cov().values

    # Generate simulations using the multivariate normal distribution
    try:
        # simulations shape: (n_sims, horizon, num_assets)
    ...
    # Compute portfolio returns for each simulation (dot product with asset weights)
    portfolio_returns = np.dot(simulations, weights)  # shape: (n_sims, horizon)
    
    # Calculate cumulative portfolio returns: starting value = 1
    portfolio_cum_returns = (1 + portfolio_returns).cumprod(axis=1)
    
    # Create and return a DataFrame where columns represent each period (e.g., Month_1, Month_2, ..., Month_horizon)
    sim_df = pd.DataFrame(portfolio_cum_returns, columns=[f"Month_{i+1}" for i in range(horizon)])
    return sim_df
def performance_attribution(aligned_returns: pd.DataFrame, weights: list) -> pd.DataFrame:
    """
    Calculate the contribution of each asset to portfolio return and variance.

    Parameters:
    -----------
    aligned_returns : pd.DataFrame
        DataFrame where each column represents an asset's historical returns.
    weights : list
    ...
    --------
    pd.DataFrame
        A DataFrame containing each asset's return contribution, variance contribution, and
        percentage of the total return.
    """
    # Calculate weighted average return per asset
    asset_returns = aligned_returns.mean() * weights
    ...
    # Calculate variance contribution for each asset
    var_contrib = []
    for i in range(len(weights)):
        contr = weights[i] * np.dot(cov_matrix.iloc[i, :], weights) / portfolio_variance
        var_contrib.append(contr)
        
    attribution_df = pd.DataFrame({
        "Asset": aligned_returns.columns,
        "Return Contribution": asset_returns,
        "Variance Contribution": var_contrib
    })
    attribution_df["Cumulative Return %"] = (attribution_df["Return Contribution"] / total_return) * 100
    return attribution_df
def run_factor_regression(portfolio_returns: pd.Series, factor_returns: pd.Series) -> dict:
    """
    Perform a linear regression of portfolio returns against factor returns.
    ...
    factor_returns : pd.Series
        Series of factor returns (e.g., benchmark).
    
    Returns:
    --------
    dict
        A dictionary with keys "Alpha", "Beta", and "R-squared" from the regression.
    """
    # Align the time indices
    ...
    return {"Alpha": model.params[0], "Beta": model.params[1], "R-squared": model.rsquared}
        ...
        try:
            ...
            with st.expander("Additional Visualizations", expanded=True):
                ...
                if not aligned_returns.empty:
                    plot_correlation_heatmap(aligned_returns)
            ...
            with st.expander("Monte Carlo Simulation", expanded=True):
                st.subheader("How the Monte Carlo Simulation Works")
                st.write("""
                    The Monte Carlo simulation leverages historical return data to forecast potential future outcomes for your portfolio.
                    It calculates the average returns and covariance among assets, then uses these statistics to randomly generate numerous 
                    future return scenarios (assuming a multivariate normal distribution). Each simulation represents a possible trajectory 
                    for portfolio performance over a specified forecast horizon, with returns compounded to show cumulative growth.

                    By running thousands of simulations, the tool creates a distribution of potential final portfolio values, enabling you to:
                    - Assess the range of outcomes and quantify risk (e.g., estimate Value-at-Risk).
                    - Understand the probability of extreme losses or gains.
                    - Gain a forward-looking perspective that complements historical performance analysis.

                    Use the input options to adjust the forecast horizon and number of simulation runs, then review the resulting charts and metrics
                    to evaluate potential future performance and risk.
                """)
                st.subheader("Monte Carlo Simulation for Portfolio Returns")
                horizon = st.number_input("Forecast Horizon (months)", min_value=6, max_value=60, value=12, step=1)
                n_sims = st.number_input("Number of Simulation Runs", min_value=100, max_value=10000, value=1000, step=100)
                
                # Ensure that your aligned asset returns (from your helper function) are available
                aligned_returns = align_asset_data(asset_data, asset_names)
                
                if aligned_returns.empty:
                    st.error("Cannot perform Monte Carlo simulation because asset return data is unavailable.")
                else:
                    try:
                        sim_df = monte_carlo_simulation_portfolio(
                        ...
                        # Prepare data for plotting sample simulation paths:
                        sim_df_reset = sim_df.reset_index().rename(columns={'index': 'Simulation'})
                        sim_melt = sim_df_reset.melt(id_vars="Simulation", var_name="Month", value_name="Cumulative Return")
                        # Convert the Month column from strings (e.g., "Month_1") to integer values
                        sim_melt["Month"] = sim_melt["Month"].str.replace("Month_", "").astype(int)
                        
                        line_chart = alt.Chart(sim_melt).mark_line(opacity=0.2).encode(
                            x=alt.X("Month:Q", title="Month"),
                            y=alt.Y("Cumulative Return:Q", title="Portfolio Value"),
                            color=alt.Color("Simulation:N", legend=None)
                        ).properties(title="Sample Simulation Paths")
                        st.altair_chart(line_chart, use_container_width=True)
                        
                        # Plot histogram of final portfolio values
                        final_values = sim_df.iloc[:, -1]
                        hist_df = pd.DataFrame({"Final Portfolio Value": final_values})
                        hist_chart = alt.Chart(hist_df).mark_bar().encode(
                            x=alt.X("Final Portfolio Value:Q", bin=alt.Bin(maxbins=30), title="Final Portfolio Value"),
                            y=alt.Y("count()", title="Frequency")
                        ).properties(title="Distribution of Final Portfolio Values")
                        st.altair_chart(hist_chart, use_container_width=True)
                        
                        # Display key metrics from the simulation
                        st.write("**Simulation Metrics**")
                        st.write(f"Mean Final Portfolio Value: {final_values.mean():.2f}")
                        st.write(f"Median Final Portfolio Value: {final_values.median():.2f}")
                        st.write(f"5th Percentile (VaR): {final_values.quantile(0.05):.2f}")
                        st.write(f"95th Percentile: {final_values.quantile(0.95):.2f}")
                    except Exception as e:
                        st.error(f"Error during Monte Carlo simulation: {e}")
            
            
            with st.expander("Stress Testing & Scenario Analysis", expanded=True):
                st.subheader("Simulate a Stress Scenario")
                st.write("""
                This section allows you to simulate a one-time shock to your portfolio.
                For example, entering a shock of -0.20 represents a 20% drop in the portfolio value at the start.
                The resulting chart shows how the portfolio would have performed following such an adverse event.
                """)
                shock = st.number_input("Enter Shock (e.g., -0.20 for a 20% drop)", 
                                        min_value=-1.0, max_value=0.0, value=-0.20, step=0.01)
                if not portfolio_rets.empty:
                    stressed_returns = portfolio_rets.copy()
                    stressed_returns.iloc[0] += shock
                    st.line_chart((1 + stressed_returns).cumprod())
                    st.write("Cumulative portfolio performance after applying the stress shock.")
                else:
                    st.warning("Portfolio returns are unavailable for stress testing.")
            
            with st.expander("Performance Attribution", expanded=True):
                st.subheader("Asset Contribution to Return and Risk")
                aligned_returns = align_asset_data(asset_data, asset_names)
                if not aligned_returns.empty:
                    attr_df = performance_attribution(aligned_returns, asset_weights)
                    st.dataframe(attr_df)
                    pie_chart = alt.Chart(attr_df).mark_arc().encode(
                        theta=alt.Theta("Variance Contribution:Q", title="Contribution to Variance"),
                        color=alt.Color("Asset:N", legend=alt.Legend(title="Asset"))
                    ).properties(title="Variance Contribution by Asset")
                    st.altair_chart(pie_chart, use_container_width=True)
                else:
                    st.error("Aligned asset returns data is unavailable.")
            
            
            with st.expander("Factor Analysis / Regression", expanded=True):
                st.subheader("Regression Analysis vs. Benchmark")
                if not portfolio_rets.empty and not benchmark_data.empty:
                    regression_results = run_factor_regression(portfolio_rets, benchmark_returns)
                    st.write("Regression Results:")
                    st.write(f"Alpha: {regression_results['Alpha']:.4f}")
                    st.write(f"Beta: {regression_results['Beta']:.4f}")
                    st.write(f"R-squared: {regression_results['R-squared']:.4f}")
                else:
                    st.warning("Portfolio returns or benchmark data are unavailable for regression analysis.")
